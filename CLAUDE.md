...
- `oauth_server.go` — OAuth 2.1 AS 本体。メタデータ・`/authorize`・`/token`・`/register`（DCR）を実装。refresh_token rotation、family tracking、replay detection を含む
...
`store.go` で定義。セッション・認可コード・アクセストークン・refresh_token・動的登録クライアントの永続化を抽象化。refresh_token 操作系メソッド（`SetRefreshToken`・`GetRefreshToken`・`ConsumeRefreshToken`・`SetFamilyRevocation`・`IsFamilyRevoked`）を追加（v0.3.0）。
...
- **refresh_token rotation**: OAuth 2.1 §4.3.2 準拠。使用時に `Used=true` に atomic 更新（削除ではなく）することで replay 検知時も `FamilyID` を取得可能にする。同一 family の全 refresh_token は `familyrevoked:<id>` tombstone で一括無効化。成功時は `slog.Info("oauth refresh rotation", "family_id", ..., "client_id", ..., "scope", ...)`、replay 検知時は `slog.Warn("oauth refresh replay detected", ...)` を出力（v0.3.1 以降）。本番 DynamoDB 観察時は projection に `used` 属性を含めることで live / rotation 済みを判別可能
