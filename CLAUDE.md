...
- **refresh_token rotation**: OAuth 2.1 §4.3.2 準拠。使用時に `Used=true` に atomic 更新（削除ではなく）することで replay 検知時も `FamilyID` を取得可能にする。同一 family の全 refresh_token は `familyrevoked:<id>` tombstone で一括無効化
